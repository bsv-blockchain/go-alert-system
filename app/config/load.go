...
func LoadConfigFile() (_appConfig *Config, err error) {
	...
	if _appConfig.LogOutputFile != "" {
		writer, err = os.OpenFile(_appConfig.LogOutputFile, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0o600)
		if err != nil {
			return nil, err
		}
	}
...
func (c *Config) createPrivateKeyDirectory() error {
	...
	if err = os.Mkdir(fmt.Sprintf("%s/%s", dirName, LocalPrivateKeyDirectory), 0o750); err != nil && !errors.Is(err, os.ErrExist) {
